...
LRESULT CMainFrame::on_user_retrieve_edit_record( WindowsMessage &message)
{
	...
	case MATCH_DISPLAY_STATE:
		// We are displaying matches now.
		// Since the content displayed has been generated by the match_maker class
		// as the marked_up_query/source/trans, we must re-calculate the score
		// in order for the edits to be reflected (also, we may have a new
		// score/matching to display)
...
/** Toggle edit mode in match state.
 */
void CMainFrame::handle_match_edit( bool edit_mode_enabled ) 
{
	if ( ! edit_mode_enabled ) // we are entering edit mode
		handle_enter_edit_mode_match() ;
	else
		handle_leave_edit_mode_match() ;
}

/** Toggle edit mode in concordance state.
 */
void CMainFrame::handle_concordance_edit( bool edit_mode_enabled ) 
{
	if ( ! edit_mode_enabled ) // we are entering edit mode
 ...
 */
void CMainFrame::handle_enter_edit_mode_match()
{
	user_feedback( IDS_ENTERING_EDIT_MODE ) ;

	m_view_interface.handle_enter_edit_mode_match( m_trans_matches ) ;

	user_feedback( IDS_IN_EDIT_MODE ) ;

}

/** Enter edit mode from concordance state.
 */
void CMainFrame::handle_enter_edit_mode_concordance()
{
	user_feedback( IDS_ENTERING_EDIT_MODE ) ;
...
/** Leaving edit mode from match state.
 */
void CMainFrame::handle_leave_edit_mode_match()
{
	...
	if ( get_display_state() == NEW_RECORD_DISPLAY_STATE )
	{
		...
		if ( m_new_record->is_valid_record() ) 
		{
			show_new_record() ;
			return ;
		}
	...
	else
	{
		...
		{
			search_match_ptr match = m_trans_matches.at( i ) ;
			if ( i == m_trans_matches.current_pos() ) 
			{
				redo_lookup( match, true ) ;
			}
			else
			{
				redo_lookup( match, false ) ;
			}
		}
...
/** Leaving edit mode from concordance state.
 */
void CMainFrame::handle_leave_edit_mode_concordance()
{
...
void CMainFrame::init_trans_matches_for_lookup( const wstring & query )
{
	...
	init_lookup_properties(m_properties, m_trans_matches.m_params);
}
...
/** Show this page when a record has been added.
 */
bool CMainFrame::show_new_record( )
{
	ATLASSERT( get_display_state() == NEW_RECORD_DISPLAY_STATE ) ;
#ifdef UNIT_TEST
	return true ;
#endif
	...
	// the presenter
	content_presenter::CContentPresenterAddedTrans presenter( m_view_interface, m_new_record ) ;
