...
#define DELCARE_MATCH( match_name, source, trans) \
	...
	{
		ASSERT_TRUE(memory_engine::is_asian(L'自')) ;
		ASSERT_TRUE(memory_engine::is_asian(L'ら')) ;
		ASSERT_TRUE(memory_engine::is_asian(L'カ')) ;
		ASSERT_TRUE(memory_engine::is_asian(L'。')) ;
		ASSERT_TRUE(memory_engine::is_asian(L'ー')) ;
	}
	...
	{
		ASSERT_TRUE(memory_engine::has_asian(L"自動車に乗る")) ;
		ASSERT_TRUE(memory_engine::has_asian(L"This has 一つ j-word")) ;
	}
	...
	TEST( detect_match_algo, char_algo )
	{
		...
		ASSERT_TRUE(IDC_ALGO_CHAR == memory_engine::detect_match_algo(L"自動車に乗る")) ;
	}
