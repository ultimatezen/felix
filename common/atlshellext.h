...
public:
   ...
   void Concatenate(LPCITEMIDLIST pidl2)
   {
      ...
      LPITEMIDLIST pidlNew = (LPITEMIDLIST) malloc(cb1 + cb2);
      if( pidlNew != NULL ) {
   ...
   { 
      ...
      pidlTarget = (LPITEMIDLIST) malloc(cbSource) ; 
...
public:
   ...
   CPidlList(HWND hwndList, DWORD dwListViewMask = LVNI_SELECTED)
   {
      ...
      LPITEMIDLIST* pidls = (LPITEMIDLIST*) malloc(nCount * sizeof(LPITEMIDLIST));   
   ...
   HRESULT Copy(LPCITEMIDLIST* pidlSource, UINT nCount)
   {
      ...
      Delete();
      m_pidls = (LPITEMIDLIST*) malloc(nCount * sizeof(LPITEMIDLIST));
   ...
   void Delete()
   {
      ...
      for( UINT i = 0; i < m_nCount; i++ ) free( m_pidls[i] ) ;
      free( m_pidls ) ;
      m_pidls = NULL;
      m_nCount = 0;
   }
   ...
   HRESULT Filter(IShellFolder* pFolder, DWORD dwItemMask)
   {
      ...
      LPITEMIDLIST* pidls = (LPITEMIDLIST*) malloc(m_nCount * sizeof(LPITEMIDLIST));
