...
{
    const unsigned char minval = min(GetRValue(rgb), min(GetGValue(rgb), GetBValue(rgb)));
    const unsigned char maxval = max(GetRValue(rgb), max(GetGValue(rgb), GetBValue(rgb)));
    ...
    if ( maxval != minval )
    { 
        const float rnorm = (maxval - GetRValue(rgb)  ) / mdiff;      
        const float gnorm = (maxval - GetGValue(rgb)) / mdiff;
        const float bnorm = (maxval - GetBValue(rgb) ) / mdiff;   
...
{
    const float hue        = ((int)HLS_H(hls)*360)/255.0f;
    const float luminance  = HLS_L(hls)/255.0f;
    const float saturation = HLS_S(hls)/255.0f;
...
CBufferDC::~CBufferDC ()
{
    ATLVERIFY(::BitBlt (m_hDestDC, m_rect.left, m_rect.top, m_rect.Width(), m_rect.Height(), m_hDC, m_rect.left, m_rect.top, SRCCOPY));
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
CBrushDC::CBrushDC (HDC hDC, const COLORREF crColor) : m_hDC (hDC)
{
...
///////////////////////////////////////////////////////////////////////////////
void CBrushDC::Color (const COLORREF crColor)
{
    ::SelectObject (m_hDC, m_hOldBrush);
