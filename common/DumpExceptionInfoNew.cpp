...
void DumpSystemInformation(file::file &log_file)
{
	...
	file::directory long_dir( CString(szModuleName), true ) ;
								...
								LPCSTR lpszMessage)
{
	...
	try
	{
		...
		module_path.GetModulePath( _AtlModule.GetResourceInstance() ) ;
		...
		if ( ! log_file.is_open() )
		{
			OutputDebugString(_("Error creating exception report\r\n"));
			return false ;
		}
					...
					Exception->NumberParameters >= 2)
		{
			...
			OutputDebugString(_("Exception handler: "));
