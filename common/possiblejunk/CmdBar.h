...
class ATL_NO_VTABLE CSmartCommandBarCtrlImpl : public CWindowImpl< T, TBase, TWinTraits>
{
public:
...
// Message map and handlers
	BEGIN_MSG_MAP(CSmartCommandBarCtrlImpl)
...
// public API handlers - these stay to support chevrons in atlframe.h
	...
	LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		if(s_pmapMsgHook == NULL)
		{
			...
			ATLASSERT( _CrtIsValidPointer( (const void*)s_pmapMsgHook, sizeof(CMsgHookMap), TRUE ) && "The allocated memory must be valid" ) ;
		}
...
//		int iSideBarWidth = rct.right-rct.left;
		...
		if (lpstrSidebarText)
		{
			...
			dc.SetTextColor( 0x0 );
			dc.DrawText(lpstrSidebarText, _tcsclen(lpstrSidebarText),
				&dims,
...
class CSmartCommandBarCtrl : public CSmartCommandBarCtrlImpl<CCommandBarCtrl>
{
public:
	DECLARE_WND_SUPERCLASS(_T("SmartCommandBar"), GetWndClassName())
};
