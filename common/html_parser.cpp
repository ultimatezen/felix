...
void html_parser::pop_tag( const wstring &tag )
{
	const wstring tag_to_pop = boost::to_lower_copy( tag ) ;
	...
	{
		unsigned int values = m_font_pop.top() ;
		m_font_pop.pop() ;
		pop_values( values ) ;
	}
	else if ( tag_to_pop == L"span" )
	{
		unsigned int values = m_span_pop.top() ;
		m_span_pop.pop() ;
		pop_values( values ) ;
	}
	else if ( tag_to_pop == L"p" )
	{
		unsigned int values = m_p_pop.top() ;
		m_p_pop.pop() ;
		pop_values( values ) ;
	}
...
void html_parser::parse_font( const wstring &attributes ) 
{
...
void html_parser::parse_span( const wstring &attributes ) 
{
	int pop_values = POP_NONE ;
...
void html_parser::parse_paragraph( const wstring &attributes ) 
{
	int pop_values = POP_NONE ;
	...
	listener->apply_paragraph( ) ;
}

bool html_parser::parse_attribute( textstream_reader<wchar_t> &reader, wstring &key, wstring &val )
{
...
void html_parser::parse_text() 
{
	...
	if ( text.empty() == false )
	{
		listener->write_text( strip_tags( text ) ) ;
	}
}

void html_parser::parse_tag()
{
	if ( m_reader.empty() )
	{
		return ;
