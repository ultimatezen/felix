...
class ScintillaModule
{
	//! Handle of the loaded scilexer.dll
	HMODULE scidll;
public:
	static ScintillaModule& instance()
	{
		static ScintillaModule module ;
		return module ;
	}
	bool IsLoaded()
	{
...
class CScintillaIF 
{
	...
	public:
		...
		/*virtual*/ inline long SPerform(long Msg, WPARAM wParam=0, LPARAM lParam=0)
		{
			...
			{
				LPARAM result = ::SendMessage(pT->m_hWnd, Msg, wParam, lParam) ;
				return static_cast< long >( result ) ;
			}
		}

		/// Load a file from "filename".
		virtual bool OpenFile(LPCSTR filename)
		{
			FILE *fp ;
			...
			if (fp) 
			{
				...
				while (lenFile > 0) 
				{
					SPerform(SCI_ADDTEXT, static_cast< WPARAM >( lenFile ), reinterpret_cast< LPARAM >( data ) );
		...
		virtual bool SaveFile(LPCSTR filename)
		{
			FILE *fp ;
	...
	public:
		...
		{
			SPerform(SCI_STYLESETFONT, (long)style, reinterpret_cast< LPARAM >(fontName) );
		}
		...
		{
			return (int)SPerform(SCI_GETSELTEXT, 0, reinterpret_cast< LPARAM >( text ) );
		}
		...
		{
			return (int)SPerform(SCI_GETTEXTRANGE, 0, reinterpret_cast< LPARAM >( tr ) );
		}
		...
		{
			SPerform(SCI_REPLACESEL, 0, reinterpret_cast< LPARAM >( text ) );
		}
		...
		{
			SPerform(SCI_SETTEXT, 0, reinterpret_cast< LPARAM >( text ) );
		}
		...
		{
			return (int)SPerform(SCI_GETTEXT, (long)length, reinterpret_cast< LPARAM >( text ) );
		}
		...
		{
			SPerform(SCI_SETKEYWORDS, (long)keywordSet, reinterpret_cast< LPARAM >(keyWords));
		}
